...
for table_type in ['balances', 'open_items', 'payment_plan']: 
    if calls_dicts[table_type]: 
        pre_pandas = pd.concat(calls_dicts[table_type].values())
    ...
    ts_name = str_snake_to_camel(table_type) + 'TS'
    ...
    pre_spark.write.format('delta').mode('append').save(table_loc)
